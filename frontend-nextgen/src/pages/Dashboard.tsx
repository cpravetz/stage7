...
const Dashboard = () => {
  ...
  const ensureConnected = useFeedStore((state) => state.ensureConnected);
  ...
  const loadServices = useCallback(async () => {
    try {
      const data = await fetch(`/api/gateway/services`).then((r) => r.json());
      const list = (data.services || []) as ServiceInfo[];
      const withHealth = await Promise.all(
        list.map(async (svc) => {
          try {
            const res = await fetch(`/api/gateway/services/${svc.id}/health`, {
              method: 'GET',
            });
            if (res.ok) {
              ...
              return {
                ...
                lastChecked: health.lastChecked || Date.now(),
              };
            }
          } catch {
            // ignore
  ...
  const formatStarted = (m: {
    startedAt?: string;
    timestamp?: number;
  }) => {
    const raw =
      m.startedAt || (m.timestamp ? new Date(m.timestamp).toISOString() : undefined);
    if (!raw) return '—';
  ...
  return (
    <div className="page">
      ...
      <div className="meta-grid" style={{ marginBottom: '1.5rem' }}>
        ...
        <div className="card" style={{ textAlign: 'center' }}>
          <div className="metric" style={{ color: '#f59e0b' }}>
            {runningMissions}
          </div>
          <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '4px' }}>
            Missions Running
        ...
        <div className="card">
          ...
          >
            ...
            <span
              ...
              style={{ fontSize: '12px' }}
            >
              {missionsLoading && !missionsLoaded ? 'Syncing...' : feedConnected ? 'Live' : 'Offline'}
            </span>
          </div>
          <div
            style={{
          ...
          >
            ...
            <span style={{ color: '#22c55e' }}>
              <strong>{runningMissions}</strong> Running
            </span>
            <span style={{ color: '#38bdf8' }}>
              <strong>{completedMissions}</strong> Completed
            </span>
            <span style={{ color: '#ef4444' }}>
              <strong>{failedMissions}</strong> Failed
            </span>
          </div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        ...
        {feedConnected ? (
          ...
          <div className="empty-state">
            Event feed not connected.{' '}
            <Link to="/feeds">Connect to live feed</Link>
          </div>
        )}
      </div>
          ...
          <>
            ...
            >
              ...
              <span style={{ color: '#f59e0b' }}>
                <strong>{degradedCount}</strong> Degraded
              </span>
              <span style={{ color: '#ef4444' }}>
                <strong>{unhealthyCount}</strong> Offline
              </span>
            </div>
            <div className="grid">
              {services.map((svc) => (
