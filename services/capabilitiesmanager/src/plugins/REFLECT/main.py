...
def call_brain(prompt: str, inputs: Dict[str, Any]) -> str:
    """Calls the Brain service to get an LLM response."""
    brain_url = os.environ.get('BRAIN_URL', 'http://brain:5070')
    ...
    try:
        response = requests.post(f"{brain_url}/chat", json=payload, headers=headers, timeout=90)
        response.raise_for_status()
        return response.json()['result']
    except requests.RequestException as e:
        logger.error(f"Brain call failed: {e}")
        raise ReflectError(f"Failed to communicate with Brain service: {e}")
