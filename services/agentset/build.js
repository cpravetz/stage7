...
try {
  console.log('Building agentset service...');
  ...
  // Create a minimal AgentSet.js file
  const minimalJsPath = path.join(distDir, 'AgentSet.js');
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', message: 'AgentSet service is running (minimal version)' });
});

app.get('/agents', (req, res) => {
  res.json({
    message: 'AgentSet service is running in minimal mode due to build issues',
    agents: []
  });
});
...
async function authenticateWithSecurityManager() {
  try {
    const response = await axios.post('http://securitymanager:5010/auth/service', {
      ...
      clientSecret: 'agentSetAuthSecret'
    });
...
async function registerWithPostOffice(token) {
  ...
  try {
    ...
    console.log('AgentSet registered with PostOffice:', response.data);
  } catch (error) {
...
app.listen(port, async () => {
  console.log(\`AgentSet (minimal version) listening on port \${port}\`);
