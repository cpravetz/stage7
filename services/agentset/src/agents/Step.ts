...
import { PluginParameterType, 
    ...
    ActionVerbTask } from '@cktmcs/shared'; // Added ActionVerbTask
...
export class Step {
    ...
    private async handleForeach(): Promise<PluginOutput[]> {
        ...
        const stepsInput = this.inputValues.get('steps'); // This should be an array of ActionVerbTask
        ...
        for (let i = 0; i < inputArray.length; i++) {
            ...
            const itemSteps: ActionVerbTask[] = JSON.parse(JSON.stringify(subPlanTemplate)); // Deep copy the plan template

            itemSteps.forEach(task => {
                // Make item and index available to sub-steps
                // We'll inject them into a special 'loop' input or modify existing inputReferences
                if (!task.inputReferences) {
                    task.inputReferences = new Map<string, InputReference>();
                }

                // A common way to provide loop variables is by prefixing them, e.g., loop.item, loop.index
                // Or by allowing direct reference if the sub-plan is designed to expect 'item' and 'index'
                // For now, let's assume the sub-plan can reference `loopItem` and `loopIndex`
                // These would need to be resolved by the Step's input population logic
                // or by modifying existing inputReferences that expect these.

                // Simplified approach: add them as direct InputValue if not referenced,
                // or expect the sub-plan to use InputReference for `loop.item` and `loop.index`.
                // For this example, let's assume the sub-plan is written to expect `loopItem` and `loopIndex`
                // as direct inputs if they are not already defined.

                // A more robust way is to iterate through task.inputReferences and substitute
                // placeholders like '{{loop.item}}' or '{{foreach.item}}'.
                // For now, we add them as potential inputs. The agent's step execution
                // logic would need to make these available if referenced.

                // To make them directly usable as input values in the sub-steps:
                // This part is tricky without knowing how sub-steps resolve their inputs.
                // A common pattern is that sub-steps would explicitly define `loopItem` or `loopIndex`
                // as one of their `inputDefinitions` if they need it.
                // Or, they might use an `InputReference` like `{{loop.item}}`.

                // Let's assume for now the plan generated by `FOREACH` will have these values
                // available for the sub-steps if they request them via `InputReference`
                // The `createFromPlan` will create steps, and the agent's `populateInputsFromDependencies`
                // would need enhancement or a new mechanism to resolve these loop-specific variables.

                // A simpler method for now: The `Agent` will handle the actual creation of `Step` objects
                // from these `ActionVerbTask`s. We are just constructing the tasks.
                // The `FOREACH` step itself can output `item` and `index` for each iteration,
                // and subsequent steps generated *by this FOREACH* can depend on these outputs.
                // This requires thinking about how `createFromPlan` handles such dynamic outputs.

                // Alternative: Each set of `itemSteps` forms a sub-plan.
                // The `FOREACH` action itself expands into a sequence of these sub-plans.
                // Each sub-plan could have an initial "SetContext" step that defines `item` and `index`.

                // Let's go with a plan: the `FOREACH` step itself will effectively "output"
                // a sequence of plans. The agent will then expand this.
                // The `result` of this `handleForeach` will be a PLAN, where each task in that plan
                // is one of the `itemSteps` with the `item` and `index` somehow embedded.

                // Simplest for now: modify the description or add a special input to each task.
                // This is not ideal for actual data flow but shows intent.
                task.description = `(Item ${i + 1}/${inputArray.length}: ${JSON.stringify(item)}) ${task.description || ''}`;

                // To make `item` and `index` available to the sub-steps,
                // we can add them to the `inputValues` of each task in the current iteration's sub-plan.
                // This assumes that `createFromPlan` will use these `inputValues` when creating the actual `Step` objects.
                // Or, more correctly, `inputReferences` should be used if these are meant to be dynamic.

                // Let's assume the sub-plan tasks can reference `loop.item` and `loop.index`
                // via their `inputReferences`. The `FOREACH` step doesn't directly create these `Step` objects.
                // It returns a plan, and the `Agent`'s `addStepsFromPlan` (which uses `createFromPlan`)
                // will convert these `ActionVerbTask`s into `Step`s.
                // The challenge is making `item` and `index` available *during* that `createFromPlan`
                // or when the `Step` populates its inputs.

                // Correct approach: The `FOREACH` handler should produce a flat list of `ActionVerbTask`s.
                // For each item in the array, it clones the `stepsInput.value` (the sub-plan).
                // Then, for each task in that cloned sub-plan, it needs to make `item` and `index` available.
                // This could be done by adding special `InputReference`s to the task that point to
                // `item` and `index` values that are implicitly provided by the FOREACH context.
                // Or, more explicitly, by transforming `InputReference`s that use placeholders.

                // For each task in the current iteration's sub-plan:
                itemSteps.forEach(subTask => {
                    if (!subTask.inputReferences) {
                        subTask.inputReferences = new Map<string, InputReference>();
                    }
                    // Provide `loop_item` and `loop_index` as constant values for this iteration's tasks
                    // These can then be referenced by the sub_task's own input mapping.
                    // Example: sub_task input "currentUser" might reference "loop_item.name" if item is an object.
                    // This requires the sub_task's inputReferences to be set up to use these.

                    // More direct: if a sub_task has an inputReference like `sourceId: 'loop', outputName: 'item'`
                    // then the input population logic needs to understand this.
                    // This is getting complex for `handleForeach` alone.

                    // Let's assume a simpler convention: the sub-plan tasks can define inputs named `loopItem` and `loopIndex`.
                    // We will try to provide these directly.
                    const currentTaskInputs = new Map<string, InputValue>();
                    currentTaskInputs.set('loopItem', { inputName: 'loopItem', value: item, valueType: PluginParameterType.ANY, args: {description: "Current item from FOREACH loop"} });
                    currentTaskInputs.set('loopIndex', { inputName: 'loopIndex', value: i, valueType: PluginParameterType.NUMBER, args: {description: "Current index from FOREACH loop"} });

                    // Merge these with existing inputReferences that might be constants
                    if (subTask.inputReferences) {
                        subTask.inputReferences.forEach((ref, key) => {
                            if (ref.value !== undefined) { // It's a constant value
                                currentTaskInputs.set(key, {inputName: key, value: ref.value, valueType: ref.valueType, args: ref.args});
                            }
                            // If it's a dependency on another step within the sub-plan, it remains.
                        });
                    }
                    // This transformation is tricky. `createFromPlan` expects `ActionVerbTask` which has `inputReferences`.
                    // `Step` objects have `inputValues` which are populated.
                    // The `ActionVerbTask` should really just define the *structure*.
                    // The `item` and `index` should be passed to `createFromPlan` somehow, or
                    // `createFromPlan` needs to be aware of a "loop context".

                    // Let's modify the task's `inputReferences` to include these as literal values.
                    // This means `createFromPlan` will see them as constants.
                    subTask.inputReferences.set('loopItem', {
                        inputName: 'loopItem',
                        value: item, // item itself
                        valueType: PluginParameterType.ANY, // Type needs to be inferred or set to ANY
                        args: { fromForeach: true }
                    });
                    subTask.inputReferences.set('loopIndex', {
                        inputName: 'loopIndex',
                        value: i,
                    ...
                    // If the sub-task already has dependencies, they should be preserved.
                    // If the sub-task references outputs from *previous tasks in the same iteration*,
                    // `createFromPlan` needs to resolve these IDs correctly within the iteration.
                    // This is handled by `createFromPlan` using relative step numbers/IDs.
                });
        ...
        if (allGeneratedSteps.length > 0) {
            return [{
                success: true,
                name: 'steps', // The Agent will see this and call addStepsFromPlan
                resultType: PluginParameterType.PLAN,
                resultDescription: `[Step] FOREACH generated ${allGeneratedSteps.length} steps.`,
                result: allGeneratedSteps // This is an array of ActionVerbTask
            }];
        } else {
             return [{ // Should only happen if input array was empty AND we didn't return earlier
                success: true,
