...
const result = await __execute_tool('career_apply_execute', {
  targetRoles,
...
console.log(JSON.stringify({ success: false, mode: 'not-connected', error: 'Not connected: no applications were submitted; provide targetRoles or select specific roles to apply to' }));
return;
}
...
name: 'Apply to Jobs',
description: 'Applies to a selected job or set of jobs using the board attached to each posting, with optional custom materials. Delegates to career_apply_execute. Human review is still recommended before sending where required.',
manifest: {
language: 'javascript',
entrypoint: 'index.js',
