...
const modelingAnalysisInputSchema = createSchemaRecord({
  operation: SchemaProps.select(['build-model', 'analyze-investment'], { description: 'Operation type: build-model for financial modeling, analyze-investment for investment analysis' }),
...
const modelingAnalysisOutputSchema = createSchemaRecord({
...
const modelingAnalysisSkill = createCodeSkill({
  id: 'finance-modeling-analysis',
  name: 'Financial Modeling & Analysis',
  description: 'Build comprehensive financial models with revenue projections, FCFF, NPV, IRR, and sensitivity analysis; analyze investments with risk metrics, VaR, Sharpe/Sortino ratios, Monte Carlo percentiles, and scenario analysis.',
  manifest: {
    language: 'javascript',
    entrypoint: 'index.js',
    sourceCode: `
const input = __tool_input || {};
const operation = input.operation || 'build-model';
...
if (operation === 'build-model') {
  ...
  console.log(JSON.stringify({ success: true, operation: 'build-model', model }));
} else {
  ...
  console.log(JSON.stringify({ success: true, operation: 'analyze-investment', analysis }));
}
`,
  },
  inputSchema: modelingAnalysisInputSchema,
  outputSchema: modelingAnalysisOutputSchema,
});

modelingAnalysisSkill.triggers = [
  { kind: 'user', phrase_examples: ['Build a financial model', 'Analyze an investment', 'Run scenario analysis'] },
  { kind: 'schedule', cadence: 'Monthly financial close digest' },
  { kind: 'schedule', cadence: 'Quarterly forecasting cadence' },
  { kind: 'event', on: 'New transaction recorded' },
  { kind: 'event', on: 'Cash burn threshold exceeded' },
];

export { modelingAnalysisSkill };
