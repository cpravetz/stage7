...
const ANALYTICS_INPUT_SCHEMA = {
  type: 'object',
  properties: {
    mode: SchemaProps.select(['report', 'trends', 'query'], { description: 'Analysis mode: metric report, trend analysis, or warehouse query and explanation', default: 'report' }),
...
const ANALYTICS_OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    mode: { type: 'string', enum: ['dry-run', 'local', 'live', 'not-connected', 'error'], description: 'Execution and connector state' },
  ...
  required: ['success', 'mode', 'source', 'warehouseConnected', 'connectorStatus', 'data', 'error', 'note'],
};
...
const BUSINESS_INSIGHT_REPORT = createCodeSkill({
  ...
  triggers: [
    { kind: 'user', phrase_examples: ['Report on metric X for period Y', 'Analyze trends for dataset X', 'Explain this business query'] },
    { kind: 'schedule', cadence: 'Weekly executive KPI trends summary' },
    { kind: 'event', on: 'New metric data lands or a warehouse data source is connected' },
    { kind: 'data', condition: 'An anomaly severity or alert threshold is crossed' },
  ],
});
