...
// Mock LLM calls endpoint
app.get('/getLLMCalls', (req, res) => {
  res.json({ llmCalls: 42 });
});

// Mock chat endpoint
app.post('/chat', (req, res) => {
  const { exchanges, optimization, conversationType } = req.body;
  ...
  const response = {
    model: 'gpt-4',
    response: `This is a mock response to: "${exchanges[exchanges.length - 1].content}"`,
