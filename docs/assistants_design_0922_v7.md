# Assistant & Skill Design — v7 (Definitive)

> This is a complete, self-contained specification. It does not require any earlier revision to be read alongside it.
> **What changed from v6, and why:** a compliance audit against v6 surfaced a real defect in v6 itself — in 14 of 21 Assistant tables, the **Description** column had been silently overwritten with only the Skill's **Tier** (Advise/Aid/Represent), losing the actual description; in the other 7, Tier was never recorded at all. This bug originated in the source document this specification was rebuilt from and survived three revisions uncoated. This revision fixes it everywhere: every Skill now has both a real Description and an explicit Tier, in separate columns. Reviewing the audit also surfaced three more §0.9 violations the audit itself missed, an over-broad application of §0.6, and one open verification item that had gone silently unresolved — all fixed or flagged below.
...
Domain Knowledge, Persistent Data, and Interfaces are properties of the Assistant, stated once. They are not repeated, word for word, in every Skill's row — repetition of this kind is exactly the pattern that produced the `CAREER_HOME` copy-paste bug across six different files: one row gets corrected, the copies don't, and nobody notices. **Note the limit of this principle:** it applies to facts that are identical across every Skill in an Assistant. A Skill's own Description and Tier are not Assistant-level facts — they are specific to that Skill and must appear on every row. Collapsing a per-Skill field into a single value across an entire Assistant (as happened to the Description column in the prior revision) is the same mistake in the opposite direction: an Assistant-level shortcut applied to row-level data, silently destroying it.
...
**Clarification, added after a compliance audit over-applied this principle:** the test is *whether a sane universal default exists*, not *whether the field is required at all*. Job Search's board list failed §0.6 because a working default (a hard-coded list of public boards) was available and simply wasn't used. A Skill that takes a real, confirmation-gated action against a specific organization's own system — applying infrastructure remediation to *this* company's cloud account, publishing to *this* company's CMS — has no universal default to fall back to; `endpointUrl` and an auth token for that specific system are a legitimate, necessary human decision, not mechanics. Required config is only a §0.6 violation when a workable default was available and skipped. Flag it as a violation only when you can name what the default should have been.
...
- **Advise** — the Skill produces analysis, recommendations, or an assessment. It reads and reasons; it does not act and does not hand the user a work product they'll send elsewhere.
- **Aid** — the Skill produces something the user will use or send themselves (a draft, a plan, a template, a co-created work product). It does work, but the last step — sending, publishing, submitting — is still the person's.
- **Represent** — the Skill acts on the user's behalf against a real external system, or completes the send/submit/publish/apply step itself. Every Represent-tier Skill **must** carry `confirmBeforeSend` (or an equivalent explicit confirmation gate) — this is enforced by type, not left as a convention each Skill remembers to set.

Tier is recorded per Skill in Part B, in its own column, never folded into or substituted for the Skill's Description — a Skill needs both a plain-language description of what it does and a tier classification of what kind of thing it's allowed to do; neither one is a substitute for the other (§0.5).
...
3. It has no `configSchema` field that is `required` but unsatisfiable by a normal user in the current environment, and no required field flagged as a §0.6 violation without a named default (§4).
...
7. Its Tier is set, and if that Tier is Represent, `confirmBeforeSend` (or equivalent) is present (§0.15).
...
**Contrast (a case this is not):** `cto-engineering-action-iac-drift-remediation` requiring `endpointUrl` and `token` is not the same failure — there is no equivalent hard-coded default for "which cloud account," and the fix here is not to remove the requirement but to leave it as a legitimate human decision (§0.6).
...
| ID | Description | Tier | Trigger | Inputs | Config | Consumes | Produces | Design |
|---|---|---|---|---|---|---|---|---|
| career-job-market-positioning-evaluator | Review resume vs listings for resume edits and salary targets | Advise *(inferred)* | **Schedule** — ongoing market-fit monitoring | systems: profile context; listings: ranked jobs | empty | career_profile_intake, career_job_discovery, career_rank | resume edits, target salary, positioning recs | MATCH |
...
| career-pipeline-outcome-tracker | Track applications and outcomes | Aid *(inferred)* | **Event** — fired by an application status change | targetRole, jobTitle, company, status, feedback, offerDetails | empty | career_pipeline_report, career_outcome | pipeline reports, outcome records | MATCH |
